...
def test_tile_manager_detects_workspace_edge_zones():
    zones = _run_tile_case()

    assert zones["fullscreen"]["name"] == "fullscreen"
    assert zones["maximize"]["name"] == "maximize"
    # This fork's tiling maps the top edge to maximize/fullscreen (asserted
    # above) rather than a top-half zone: the corner boxes own the upper edge
    # and the mid-upper strip is intentionally free, so a pointer at (500, 20)
    # resolves to no zone. (Upstream's all-edge model returned a top-half here;
    # the fork replaced it — see tileManager._zoneForPointer.)
    assert zones["top"] is None
    assert zones["left"] == {
        "name": "left-half",
...
def test_regular_tool_modals_are_not_limited_to_fullscreen_only():
    ...
    # Settings is unrestricted in this fork (per-modal tiling limits were
    # removed); (500, 20) resolves to no zone for the same reason as the `top`
    # case above, so this stays None — now by absence of a zone, not a guard.
